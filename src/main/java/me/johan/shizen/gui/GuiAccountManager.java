package me.johan.shizen.gui;

import me.johan.shizen.Shizen;
import me.johan.shizen.auth.Account;
import me.johan.shizen.auth.MicrosoftAuth;
import me.johan.shizen.auth.SessionManager;
import me.johan.shizen.utils.Notification;
...
public class GuiAccountManager extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float renderPartialTicks) {
    ...
    drawCenteredString(
      ...
      TextFormatting.translate(String.format(
        "&rAccount Manager &8(&7%s&8)&r", Shizen.accounts.size()
      )),
      width / 2, 20, -1
    );
  ...
  protected void keyTyped(char typedChar, int keyCode) {
    switch (keyCode) {
      ...
      case Keyboard.KEY_DOWN: {
        if (selectedAccount < Shizen.accounts.size() - 1) {
          ++selectedAccount;
          if (isCtrlKeyDown()) {
            Collections.swap(Shizen.accounts, selectedAccount, selectedAccount - 1);
            Shizen.save();
    ...
    if (isKeyComboCtrlC(keyCode) && selectedAccount >= 0) {
      setClipboardString(Shizen.accounts.get(selectedAccount).getUsername());
    }
  }
      ...
      switch (button.id) {
        case 0: { // Login
          if (task == null || task.isDone()) {
            ...
            Account account = Shizen.accounts.get(selectedAccount);
  ...
  class GuiAccountList extends GuiSlot {
    ...
    protected int getContentHeight() {
      return Shizen.accounts.size() * 16;
    }
    ...
    protected void drawSlot(int entryID, int x, int y, int k, int mouseXIn, int mouseYIn) {
      ...
      Account account = Shizen.accounts.get(entryID);
